...
public class ShortestPathAPI extends TraversersAPI {
    ...
    public List<Object> get(Object sourceId, Object targetId,
                            Direction direction, String label, int maxDepth) {
        String source = GraphAPI.formatVertexId(sourceId, false);
        String target = GraphAPI.formatVertexId(targetId, false);
        E.checkArgument(direction == Direction.IN ||
                        direction == Direction.OUT,
                        "Direction for shortest path must be IN or OUT, " +
                        "but got '%s'", direction);
        E.checkArgument(maxDepth >= 1,
                        "Max depth of shortest path must >= 1, " +
                        "but got '%s'", maxDepth);
        ...
        return result.readList("path", Object.class);
    }
}
